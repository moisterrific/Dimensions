...
class ClientPacketHandler {
  ...
  private runPriorHandlers(client: Client, packet: Packet): boolean {
  ...
  private runPostHandlers(client: Client, packet: Packet): boolean {
    let handlers = client.globalHandlers.extensions;
    let handled = false;
    for (let key in handlers) {
      let handler = handlers[key];
  ...
  public handlePacket(client: Client, packet: Packet): Buffer | null {
  ...
  private handlePlayerInfo(packet: Packet): boolean {
  ...
  private handleUpdatePlayerBuff(packet: Packet): boolean {
  ...
  /* Used to prevent invisibility buff from being sent to the server
   * for used when the config is set to blockInvis = true */
  private handleAddPlayerBuff(packet: Packet): boolean {
    const reader: PacketReader = new PacketReader(packet.data);
    let playerID: number = reader.readByte();
  ...
  private handlePlayerMana(packet: Packet): boolean {
   ...
   * SSC server to a Non-SSC server */
  private handlePlayerHP(packet: Packet): boolean {
  ...
  private handleUpdatePlayer(packet: Packet): boolean {
    const reader: PacketReader = new PacketReader(packet.data);
    reader.readByte();
    reader.readByte();
    reader.readByte();
  ...
  private handleUpdateItemOwner(packet: Packet): boolean {
    // Prevent this being sent too early (causing kicked for invalid operation)
    if (this.currentClient.state !== ClientState.FullyConnected) {
      this.currentClient.packetQueue.push(packet.data);
      return true;
  ...
  private handleSpawnPlayer(packet: Packet): boolean {
  ...
  private handleLoadNetModule(packet: Packet): boolean {
    let handled = false;
    const reader = new PacketReader(packet.data);
  ...
  private handleClientUUID(packet: Packet): boolean {
    const reader: PacketReader = new PacketReader(packet.data);
  ...
  private handleProjectileUpdate(packet: Packet): boolean {
    ...
    this.currentClient.projectileTracking.add(projectileId);
  ...
  private handleDestroyProjectile(packet: Packet): boolean {
    const reader: PacketReader = new PacketReader(packet.data);
    const projectileId = reader.readInt16();
    this.currentClient.projectileTracking.delete(projectileId);
