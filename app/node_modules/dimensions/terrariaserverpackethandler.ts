...
class TerrariaServerPacketHandler {
   ...
   */
  private runPriorHandlers(server: TerrariaServer, packet: Packet): boolean {
   ...
   * @return Whether or not the packet was handled (and should not be sent)
   */
  private runPostHandlers(server: TerrariaServer, packet: Packet): boolean {
    let handlers = server.client.globalHandlers.extensions;
    let handled = false;
    for (let key in handlers) {
      let handler = handlers[key];
   ...
   */
  public handlePacket(server: TerrariaServer, packet: Packet): Buffer | null {
    ...
    switch (packetType) {
      ...
      case PacketTypes.ProjectileUpdate:
        handled = this.handleProjectileUpdate(packet);
        break;

      case PacketTypes.DestroyProjectile:
        handled = this.handleDestroyProjectile(packet);
        break;
   ...
   */
  private handleDisconnect(packet: Packet): boolean {
   ...
   * @return Whether or not this packet was handled (and should not be sent)
   */
  private handleContinueConnecting(packet: Packet): boolean {
    let reader: PacketReader = new PacketReader(packet.data);
   ...
   * @return Whether or not this packet was handled (and should not be sent)
   */
  private handleWorldInfo(packet: Packet): boolean {
    let reader: PacketReader = new PacketReader(packet.data);
   ...
   */
  private handleCompleteConnectionAndSpawn(packet: Packet): boolean {
   ...
   * @return Whether or not the packet has been handled (and is not to be sent)
   */
  private handleDimensionsUpdate(packet: Packet): boolean {
   ...
   * @return Whether or not the packet has been handled (and is not to be sent)
   */
  private handleNPCUpdate(packet: Packet): boolean {
    let reader: PacketReader = new PacketReader(packet.data);
   ...
   * @return Whether or not this packet was handled (and should not be sent)
   */
  private handleUpdateItemDrop(packet: Packet): boolean {
    let reader: PacketReader = new PacketReader(packet.data);
   ...
   * @return Whether or not this packet was handled (and should not be sent)
   */
  private handlePlayerActive(packet: Packet): boolean {
    let reader: PacketReader = new PacketReader(packet.data);
   ...
   */  
  private handlePlayerInventorySlot(packet: Packet): boolean {
    let handled = false;
    let reader: PacketReader = new PacketReader(packet.data);
    let playerID: number = reader.readByte();
  ...
  private handleProjectileUpdate(packet: Packet): boolean {
    const reader: PacketReader = new PacketReader(packet.data);
    ...
    if (owner === this.currentServer.client.player.id) {
      this.currentServer.client.projectileTracking.add(projectileId);
    }

    return false;
  }

  private handleDestroyProjectile(packet: Packet): boolean {
    const reader: PacketReader = new PacketReader(packet.data);
    const projectileId = reader.readInt16();
    const owner = reader.readByte();
    if (owner === this.currentServer.client.player.id) {
      this.currentServer.client.projectileTracking.delete(projectileId);
    }
  ...
  private syncProjectiles(): void {
    const projectileSync = this.currentServer.client.options.synchronization.projectiles;
    if (projectileSync.enabled && (projectileSync.when === "always" || this.currentServer.isSSC)) {
      ...
      projectileIds.forEach(projectileId => {
        if (projectileSync.type === "clear") {
          this.currentServer.client.send(new PacketWriter()
              .setType(PacketTypes.DestroyProjectile)
              .packInt16(projectileId)
              .packByte(this.currentServer.client.player.id)
              .data);
          this.currentServer.client.projectileTracking.clear();
        }
      })
    }
  }
